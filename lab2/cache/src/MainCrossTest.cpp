...
Cache* l1Cache, * l2Cache, * victcache;
...
int main(int argc, char** argv) {
    simulator.isCrossTest = true;
    if (!parseParameters(argc, argv)) {
        printUsage();
        exit(-1);
    }
    //elfFile = "D:/Course/CS211/cs211-lab/lab2/cache/riscv-elf/quicksort.riscv";
    if (with_victim)
    ...
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 2; ++k) {
                ...
                victpolicy.cacheSize = 512;
                victpolicy.blockSize = 64;
                victpolicy.blockNum = victpolicy.cacheSize / victpolicy.blockSize;
